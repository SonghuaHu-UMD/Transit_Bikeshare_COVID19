...
dat <- read.csv('D:\\COVID19-Transit_Bikesharing\\Divvy_Data\\All_final_Divvy_R_BSTS_0907.csv')
...
GAM_RES1 <-
  mgcv::bam(Cum_Relt_Effect ~
    ...
    s(Time_Index, capacity), data = dat, family = c("gaussian"),
            control = gam.control(trace = TRUE),
            method = "fREML", discrete = TRUE, chunk.size=5000, nthreads = 150)
summary(GAM_RES1)
plot(GAM_RES1,scheme = 2)
...
GAM_RES1 <-
  mgcv::gam(Cum_Relt_Effect ~   Pct.White +  Pct.Asian + Pct.Car + Pct.BikeWalk + Pct.WorkHome + Cumu_Death +
    INDUSTRIAL + INSTITUTIONAL + OPENSPACE + Bike_Route +
    Pct.WJob_Utilities + Pct.WJob_Goods_Product + WTotal_Job_Density + boardings  + Distance_Busstop +
    rides + Distance_Rail  + Distance_Bikestation  + Distance_City + PopDensity + capacity +
...
GAM_RES2 <-
  mgcv::bam(Predict ~ Pct.Male + Pct.Age_0_24 + Pct.Age_25_40 + Pct.Age_40_65 + Pct.White + Pct.Black + Pct.Asian +
       Income + College + Pct.Car + Pct.BikeWalk + Pct.WorkHome + Cumu_Cases + Cumu_Death +
    COMMERCIAL + INDUSTRIAL + INSTITUTIONAL + OPENSPACE + RESIDENTIAL +  Bike_Route +
    Pct.WJob_Utilities + Pct.WJob_Goods_Product + WTotal_Job_Density + boardings  + Distance_Busstop +
